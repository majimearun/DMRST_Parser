...
def process_pdf(pdf_path):
    ...
    for i, chunk in enumerate(chunks):
        ...
        for new_section in sections:
            if new_section in all_sections:
                new_key = new_section + "_additional"
                all_sections[new_key] = sections[new_section]
            else:
                all_sections[new_section] = sections[new_section]
...
if __name__ == "__main__":
    ...
    @app.post("/parse_pdf/{pdf_name}")
    def parse_pdf(pdf_name: str):
        if not os.path.exists(f"./sections/{pdf_name}_sections.pkl"):
            sections = process_pdf(f"./pdfs/{pdf_name}")
            with open(f"./sections/{pdf_name}_sections.pkl", "wb") as f:
                pickle.dump(sections, f)

        with open(f"./sections/{pdf_name}_sections.pkl", "rb") as f:
            sections = pickle.load(f)
        return {"Sections": list(sections.keys())}

    @app.get("/original_section/{pdf_name}/{section_name}")
    def original_section(pdf_name: str, section_name: str):
        with open(f"./sections/{pdf_name}_sections.pkl", "rb") as f:
            sections = pickle.load(f)
        return {"Original Section": sections[section_name]}

    @app.get("/generate_summary/{pdf_name}/{section_name}")
    def generate_summary(pdf_name: str, section_name: str):
        with open(f"./sections/{pdf_name}_sections.pkl", "rb") as f:
            sections = pickle.load(f)

        Test_InputSentences = []
        sent = sections[section_name].replace("\n", " ")
        ...
        for index in range(len(all_segmentation_pred)):
            ...
            """
            print("created prompt, now generating summary")
            return {"Summary": run_prompt(prompt)}
